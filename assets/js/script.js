...
function initializeBoardTiles() {
    ...
    // Cities array has 12 cities, we'll use all 12 once, then reuse 4 cities
    let cityIndex = 0;
    boardTiles = [
        ...
        { name: cities[cityIndex % cities.length], type: "question" },      // 19: Philipina (reuse)
        { name: cities[(cityIndex + 1) % cities.length], type: "question" },      // 20: Thailand (reuse)
        { name: "DANA UMUM", type: "community" },            // 21
        { name: cities[(cityIndex + 2) % cities.length], type: "question" },      // 22: Jepang (reuse)
        { name: cities[(cityIndex + 3) % cities.length], type: "question" },      // 23: Korea (reuse)
    ];
}
